...
此文档提供了一个使用Perl进行面向对象编程的介绍。它以一个简短的关于面向对
象设计的概念的概述开始，之后介绍了L<CPAN|http://search.cpan.org>上基于
Perl的各种不同的OO系统。
...
默认情况下，Perl内置的OO系统非常小巧，你必须自己做大部分工作。这种小巧在
1994年那个时候是合理的，但自Perl5.0发布以来，我们看到许多通用的模式在
Perl OO里面显现。幸运的是，Perl的灵活性允许Perl OO的生态系统演化和开花。
...
大部分面向对象编程系统都有一些相同的概念。你可能已经听说过了一些像“类”，
“对象”，“方法”和“属性”的术语。理解这些概念，能够使你更加容易的阅读和编写
面向对象的代码。如果你已经熟悉了这些，你仍然可以阅读一下这一节，它用Perl
的术语解释了这些概念。
...
Perl的OO系统基于类(class-based)。基于类的OO是相当通用的，java，C++，
C#，Python，Ruby和很多其他语言都基于此。当然也有其他类型的面向对象。
Javascript就是使用另一种范式的流行语言。Javascript的OO系统是基于原型的
(prototype-based)。
...
B<对象>是聚合了数据和函数的数据结构，函数操作数据。对象的数据叫做B<属性>
，它的函数叫做B<方法>。一个对象可以想象成为一个名词(一个人，一个web服务
，一台计算机)。
...
一个对象代表了一个单一但不连续的物体（????）。比如，它可能代表一个文件。
文件对象的属性可能包括路径，内容，最后修改时间。如果我们创建一个代表
F</etc/hostname>的对象，这个文件在名为"foo.example.com"的机器上。这个对
象的路径就应该是"/etc/hostname"，内容是"foo\n"，最后修改时间是自epoch后
1304974868秒。
...
Perl的大部分对象都是hash，但我们推荐的OO系统会使你不必担心这一点。实际上
，最好将对象看做一个不透明的内部数据结构。
...
所有的对象都属于某个类。比如，我们的F</etc/hostname>对象属于C<File>类。
当我们想创建一个特定对象，我们以它的类开始，B<创建>或者B<实例化>一个对象
。对象经常被称为一个类的B<实例>。
...
同时对于Perl来说，没有一个特定的关键字来创建对象。但是，大部分CPAN上的OO
模块都使用C<new()>来创建对象。
...
如前所述，大多Perl的对象都是hash，但它也可以是任何Perl数据类型的实例(标
量，数组等)。把一个普通的Perl数据结构转换成对象的方式是使用C<bless>函数
对它B<Blessing>。
...
个术语。一个B<blessed>的数据结构(比如引用)是一个对象。我们有时候会说一个
对象被“blessed into a class”。
...
当引用被blessed，L<Scalar::Util>模块中的C<blessed>函数可以告诉我们它的类
名。当参数为对象时，这个函数返回对象的类，否则返回false。
...
B<构造器>创建新对象。不像其他语言，提供了构造器语法，在Perl中，一个类的
构造器只是一个方法。大部分Perl的类使用C<new>作为构造器的名字。
...
argument:
在Perl中，方法是类的包里面的子程序。方法的第一个参数总是对象。
...
一个假想的名字，它代表箭头左边的部分。Invocant可以是class名或者一个对象
。我们当然也可以给方法传递其他参数。
...
所有的类都可以定义它的B<属性>。当我们初始化一个对象时，我们给这些属性赋
...
Perl没有替属性定义特殊的语法。在底层，属性就是类所对应hash的键，对此没什
么可以担心的。
...
我们推荐你B<存取器>来访问属性。存取器是用来获得或设置属性的值的方法。我
们在C<print_info()>里面已经看到过了: C<< $self->path >>
...
B<多态>是来自不同类的对象公用一个API的说法。比如，C<File>和C<WebPage>类
都可以有C<print_content()>方法。对于不同的类，调用这个方法的输出可能不同
，但他们有通用的接口。
...
我们经常把继承关系称为B<父-子>或者C<超类/子类>。有时候我们说子类是一个父
类。
...
L<parent>模块是Perl让你定义继承继承关系的诸多方法之一。
...
们强烈反对这样做。通常，你可以使用B<roles>来做到用多重继承能做到的所有事
情，而且做法还能更加清晰。
...
注意对一个给定类，给它定义多个子类没有什么关系，这是通用和安全的做法。比
...
决定使用哪个方法的过程叫做B<方法解析>。Perl所做的是：先查看对象的类(这里
是C<File::MP3>)，如果类定义了这个方法，那么调用这个类的版本。否者，Perl
查看每个父类。对于C<File::MP3>，它的父类只有C<File>。如果C<File::MP3>没
有定义这个方法，而C<File>定义了，那么Perl就会调用C<File>里面的方法。

If C<File> inherited from C<DataSource>, which inherited from C<Thing>,
then Perl would keep looking "up the chain" if necessary.

如果C<File>继承自C<DataSource>，而C<DataSource>继承自C<Thing>，那么如果
必要的话，Perl会“源着继承链自下而上”的查找方法。
...
封装在许多方面都是非常重要的。首先，它允许你将公用API和内部实现分开。这
意味着你能在修改实现的同时不破坏API接口。
...
其次，如果很好的封装了类，继承也变得更加简单。理想情况是，子类使用和父类
相同的接口来访问对象数据。实际上，有时候继承会破坏封装，但好的接口会减少
这种破坏。
...
之前我们提到大部分Perl的对象都是用hash实现的。封装的原则告诉我们不应依赖
于此，而应该使用存取器来获得hash中的数据。我们接下来推荐的对象系统都能自
动生成存取器。如果你使用它们之中的任意一个，你永远不需要使用hash来访问对
象。
...
C<Computer>。这两种物体都有开关，我们想在类定义中为之建立模型。
...
我们可以让两个类都继承自同一个父类，比如C<Machine>，但是不是所有的机器都
有开关键。我们也可以建立一个叫C<HasOnOffSwitch>的父类，但这也太文绉绉了
。Radio和Computers不是这种父类的特殊体。这个父类建的有点搞笑。
...
这就是角色可以出现的地方。此时建立一个C<HasOnOffSwitch>角色并将之应用于
类显得更加合理。这个角色可以定义如C<turn_on()>和C<turn_off()>之类的借口
。
...
Perl并没有关于角色的内置接口。过去，人们都是痛苦的选择使用多重继承。而现
在，CPAN上有许多选择可以让你使用角色。
...
就像前面所说一样，Perl内置的OO系统非常小巧，也相当灵活。这些年来，在Perl
的内置系统上面，人们开发了许多高级系统，用以提供更多的特性和便利。
...
我们强烈推荐你使用这些系统中的一个。它们之中即使是最轻巧的都能简化许多重
复工作。没有任何理由从零开始用Perl构建你的类。
...
C<Moose>提供了一个完全而现代的OO系统。对它影响最大的是Common Lisp的面向
对象系统，同时它也借鉴了Smalltalk和许多其他编程语言的理念。C<Moose>由
Stevan Little创建，从他对Perl 6 OO的设计工作中获益良多。
...
C<has()>子函数定义了一个属性，Moose会自动为属性创建存取器。它还帮你创建
了C<new()>方法。这个构造器知道你所定义的属性，所以你能够在创建一个
C<File>对象时给它们赋值。
...
在上面的例子中，在创建C<is_on>属性的时候，我们给C<has()>传递了参数C<<
isa => 'Bool' >>。这是在告诉C<Moose>，这个属性必须是一个布尔值。如果我们
给它设置非法值，代码会抛出错误。
...
省层，使得你可以问诸如“File类实现了那些方法？”之类的问题。它还可以让你
按计划的修改你的类???。

=item * Self-hosted and extensible

=item * 自宿主(Self-hosted)和可扩展
...
通过阅读C<Moose>的文档来学习它，从
L<Moose::Manual|http://search.cpan.org/perldoc?Moose::Manual>开始。
...
C<Moose>会使你的代码载入时间变慢。C<Moose>不是一个小型系统，当你定义你的
类时，它做了I<许多>代码生成工作。这意味着你的代码在运行期间能尽可能的快
，但在你的模块第一次被载入时，你必须付出一些时间代价。
...
C<Moose>。我们鼓励你先试用一下C<Moose>，然后再考虑启动速度的问题。
...
C<Moose>还有很多其他模块的依赖关系，其中大部分都是小型的独立模块，还有一
部分是为了C<Moose>而写的。C<Moose>本身，以及它的一些依赖模块需要用到编译
器。如果你想将你的软件安装在一个没有编译器的系统上面，或者有I<任何>依赖关
系都是问题，那么C<Moose>可能不适合你。
...
如果你使用了C<Moose>之后，发现这些问题之一妨碍你继续使用C<Moose>，我们建
议你接下来考虑一下C<Mouse>。C<Mouse>用一个更简单的包，实现了C<Moose>功能
的一个子集。对于它所实现的所有特性，其接口和C<Moose>是I<完全相同的>，这
意味着你可以非常容易的从C<Moose>移植到C<Mouse>
...
更快。同时，它的所有依赖模块都由Perl核心提供，不需要编译器就能运行。如果
你有编译器，那么C<Mouse>会用它来编译一些代码，用来加快速度。
...
最后，它还包括了一个C<Mouse::Tiny>模块，这个模块中包含了大部分C<Mouse>的
特性，并将其打包到了一个模块文件里面。把这个文件拷贝到你应用的库目录，你
就可以轻易地将其打包。
...
C<antlers>导入符告诉C<Class::Accessor>我们想用类C<Moose>的语法定义属性。
你唯一可以传给C<has>的参数是C<is>。当你使用C<Class::Accessor>时，我推荐
你使用类Moose的语法，这意味着当你之后决定使用C<Moose>时，你可以更平滑的
升级。
...
其简单。但我们仍然认为比起自己写你的OO代码，使用这个模块更加简单。
...
我们前面提过，角色提供了继承之外的另外一个选择，但Perl没有内置的角色支持
。如果你选择使用Moose，它提供了一个成熟的角色实现。如果你使用其他我们推
荐的OO模块，你仍然可以通过L<Role::Tiny>使用角色。
...
C<Moose>是最好选择。它提供了许多特性，强大的生态系统，大量的用户基础。我
们也简单的提到了C<Mouse>，它是C<Moose>的简化版，也是当Moose不适合于你的
应用的时候的一个合理选择。
...
C<Object::Tiny>绝对是最后的选择。它没有任何依赖，不需要学习任何语法。当
你需要一个超级简单的环境，并且不需要考虑细节的将一些东西捏合起来时，它是
一个不错的选择。
...
如果你发现自己在考虑多重继承的时候，组合使用C<Role::Tiny>和
...
仍然可以选择自己手动编写你的类，但现在，2011，你没有任何理由要去那样做。
...
对于更大的项目，L<Moose>提供了丰富的特性，使你能够关注在逻辑层面上。
