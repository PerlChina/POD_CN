...
默认情况下，Perl内置的OO系统非常轻量，因此你必须自己做大部分工作。这种小
巧在1994年那个时代是合理的，但自Perl5.0发布以来，许多通用的模式在Perl OO
系统里面显现。幸运的是，Perl的灵活性允许Perl OO的生态系统演化和开花。
...
当引用被bless后，L<Scalar::Util>模块中的C<blessed>函数可以告诉我们它的类
名。当其参数为对象时，这个函数返回对象的类，否则返回false。
