...
=item * arrays of arrays 多维数组

=item * hashes of arrays 哈希数组

=item * arrays of hashes 数组哈希

=item * hashes of hashes 哈希的哈希
...
=head1 ARRAYS OF ARRAYS 多维数组
X<array of arrays> X<AoA>

=head2 Declaration of an ARRAY OF ARRAYS 声明多维数组
...
=head2 Generation of an ARRAY OF ARRAYS 构造多维数组

 # reading from file 从文件逐行读取
 while ( <> ) {
     push @AoA, [ split ];
 }
 ...
 # using temp vars 使用临时变量
 for $i ( 1 .. 10 ) {
     @tmp = somefunc($i);
     $AoA[$i] = [ @tmp ];
 }

 # add to an existing row 向已经存在的多维数组添加新元素
 push @{ $AoA[0] }, "wilma", "betty";

=head2 Access and Printing of an ARRAY OF ARRAYS 访问并打印多维数组

 # one element 给一个元素赋值
 $AoA[0][0] = "Fred";

 # another element 把另一个元素里的单词变成大写
 $AoA[1][1] =~ s/(\w)/\u$1/;

 # print the whole thing with refs 用引用打印两维数组的全部元素
 for $aref ( @AoA ) {
     print "\t [ @$aref ],\n";
 }

 # print the whole thing with indices 用索引打印两维数组的全部元素
 for $i ( 0 .. $#AoA ) {
     print "\t [ @{$AoA[$i]} ],\n";
 }

 # print the whole thing one at a time 打印所有元素，一次打印一个元素
 for $i ( 0 .. $#AoA ) {
     for $j ( 0 .. $#{ $AoA[$i] } ) {
...
=head1 HASHES OF ARRAYS 哈希数组
X<hash of arrays> X<HoA>

=head2 Declaration of a HASH OF ARRAYS 声明哈希数组
...
=head2 Generation of a HASH OF ARRAYS 构造哈希数组

 # reading from file 读取文件，每行格式为A：B C D E，用冒号分割，前面是key，后面用空格分割，多个元素作为value
 ...
 # reading from file; more temps 读取同一个文件，使用更多临时变量
 # flintstones: fred barney wilma dino
 while ( $line = <> ) {
 ...
 # calling a function that returns a list 调用函数，返回列表
 for $group ( "simpsons", "jetsons", "flintstones" ) {
     $HoA{$group} = [ get_family($group) ];
 }

 # likewise, but using temps 类似，但使用了临时变量
 for $group ( "simpsons", "jetsons", "flintstones" ) {
     @members = get_family($group);
     $HoA{$group} = [ @members ];
 }

 # append new members to an existing family 向已有的多维数组追加元素
 push @{ $HoA{"flintstones"} }, "wilma", "betty";

=head2 Access and Printing of a HASH OF ARRAYS 访问和打印哈希数组

 # one element 访问一个元素
 $HoA{flintstones}[0] = "Fred";

 # another element 替换另一个元素的值
 $HoA{simpsons}[1] =~ s/(\w)/\u$1/;

 # print the whole thing 打印全部
 foreach $family ( keys %HoA ) {
     print "$family: @{ $HoA{$family} }\n"
 }

 # print the whole thing with indices 使用索引打印全部
 foreach $family ( keys %HoA ) {
     print "family: ";
 ...
 # print the whole thing sorted by number of members （按value数组元素个数从大到小）排序并打印
 foreach $family ( sort { @{$HoA{$b}} <=> @{$HoA{$a}} } keys %HoA ) {
     print "$family: @{ $HoA{$family} }\n"
 }

 # print the whole thing sorted by number of members and name （按value数组元素个数从大到小或者key字符升序）排序并打印
 foreach $family ( sort {
			    @{$HoA{$b}} <=> @{$HoA{$a}}
					||
				    $a cmp $b
...
=head1 ARRAYS OF HASHES 数组哈希
X<array of hashes> X<AoH>

=head2 Declaration of an ARRAY OF HASHES 声明数组哈希
...
=head2 Generation of an ARRAY OF HASHES 构造数组哈希

 # reading from file 读取文件，每行格式为k1=v1 k2=v2
 ...
 # reading from file 读取文件，没有使用临时变量，用的是环境变量$_
 # format: LEAD=fred FRIEND=barney
 # no temp
 while ( <> ) {
     push @AoH, { split /[\s+=]/ };
 ...
 # calling a function  that returns a key/value pair list, like 调用函数，返回哈希（key/value）结构，例如lead:fred,daughter:pebbles
 # "lead","fred","daughter","pebbles"
 while ( %fields = getnextpairset() ) {
     push @AoH, { %fields };
 }

 # likewise, but using no temp vars 类似的，但没有使用临时变量
 while (<>) {
     push @AoH, { parsepairs($_) };
 }

 # add key/value to an element 向一个数组元素增加key/value
 $AoH[0]{pet} = "dino";
 $AoH[2]{pet} = "santa's little helper";

=head2 Access and Printing of an ARRAY OF HASHES 访问和打印数组哈希

 # one element 给数组元素的一个key赋值
 $AoH[0]{lead} = "fred";

 # another element 正则替换数组元素的key的值
 $AoH[1]{lead} =~ s/(\w)/\u$1/;

 # print the whole thing with refs 使用引用打印全部
 for $href ( @AoH ) {
     print "{ ";
     for $role ( keys %$href ) {
 ...
 # print the whole thing one at a time 打印全部，一次一个key/value
 for $i ( 0 .. $#AoH ) {
     for $role ( keys %{ $AoH[$i] } ) {
...
=head1 HASHES OF HASHES 哈希的哈希
X<hash of hashes> X<HoH>

=head2 Declaration of a HASH OF HASHES 声明哈希哈希
...
=head2 Generation of a HASH OF HASHES 构造哈希哈希value

 # reading from file 从文件逐行读取，按冒号分割取出第一级的key和value，按空格分割value取出subkey=subvalue，然后按“=”分割取subkey和subvalue。
 ...
 # reading from file; more temps 从文件读取，用了更多临时变量
 while ( <> ) {
     next unless s/^(.*?):\s*//;
     $who = $1;
     $rec = {};
 ...
 # calling a function  that returns a key,value hash 调用函数，返回哈希。
 for $group ( "simpsons", "jetsons", "flintstones" ) {
     $HoH{$group} = { get_family($group) };
 }

 # likewise, but using temps 跟上面类似，没有使用临时变量
 for $group ( "simpsons", "jetsons", "flintstones" ) {
     %members = get_family($group);
     $HoH{$group} = { %members };
 }

 # append new members to an existing family 先给哈希赋值，然后向这个哈希增加成员
 %new_folks = (
     wife => "wilma",
     pet  => "dino",
 );
...
=head2 Access and Printing of a HASH OF HASHES 访问和打印哈希哈希

 # one element 给一个key的subkey赋值
 $HoH{flintstones}{wife} = "wilma";

 # another element 正则替换一个key的subkey的值
 $HoH{simpsons}{lead} =~ s/(\w)/\u$1/;

 # print the whole thing 打印全部 key: { subkey= subvalue }
 ...
 # print the whole thing  somewhat sorted 按默认排序（quicksort）打印
 ...
 # print the whole thing sorted by number of members 按成员的成员数量降序打印
 ...
 # establish a sort order (rank) for each role 为每种角色打分排序
 $i = 0;
 for ( qw(lead wife son daughter pal pet) ) { $rank{$_} = ++$i }

 # now print the whole thing sorted by number of members 按成员的成员数量降序打印
 foreach $family ( sort { keys %{ $HoH{$b} } <=> keys %{ $HoH{$a} } } keys %HoH ) {
     print "$family: { ";
     # and print these according to rank order 指定sort排序规则，按角色评分升序打印
...
=head1 MORE ELABORATE RECORDS 更详细的数据结构示例
X<record> X<structure> X<struct>

=head2 Declaration of MORE ELABORATE RECORDS 声明复杂的数据结构
...
下面一个示例演示如何创建和使用一个复杂数据结构，会使用多种不同的排序方法.
     ...
     # careful of extra block braces on fh ref 注意句柄引用的大括号
...
=head2 Declaration of a HASH OF COMPLEX RECORDS 声明一个复杂的哈希
...
=head2 Generation of a HASH OF COMPLEX RECORDS 构造一个复杂的哈希
     ...
     # here's a piece by piece build up 下面演示如何一点点添加到数据结构里
     $rec = {};
     $rec->{series} = "flintstones";
     $rec->{nights} = [ find_days() ];

     @members = ();
     # assume this file in field=value syntax 假定文件格式是“成员=值”这样的
     while (<>) {
         %fields = split /[\s=]+/;
         push @members, { %fields };
     }
     ...
     # now remember the whole thing 赋值，记住$rec
     $TV{ $rec->{series} } = $rec;
     ...
     foreach $family (keys %TV) {
         $rec = $TV{$family}; # temp pointer 临时指针
         @kids = ();
