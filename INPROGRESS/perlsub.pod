...
调用子程序：
    ...
    NAME(LIST);    # 用圆括号的时候
    NAME LIST;     # 子程序预先有定义或者从别的模块导入的话，圆括号可以没有
    &NAME(LIST);   # 不做原型检查
    &NAME;         # 当前的@_作为被调子程序的参数

=head1 DESCRIPTION

和很多语言一样，Perl同样提供了用户自定义子程序的机制。子程序可以位于主程
序文件的任意位置，通过C<do>、C<require>或C<use>关键字从其它源文件中加
载，或者通过C<eval>在运行时动态生成，或者以匿名子程序的形式存在。甚至可
以通过值为子程序名或者值为代码引用的变量来调用子程序。
...
Perl采用了一种简单的函数调用及其返回值的模型：传给函数的参数和函数返回值
都是一个标量的列表。如果参数或返回值列表中有数组或哈希，数组的元素或哈希
的键值将会成为列表的一部分，除非传入或返回数组、哈希的引用。参数和返回值
列表的长度没有限制。（通常没有显式的返回语句的函数被称为子程序，不过从
Perl的角度来看其实没差）。
...
在函数里，传入的参数被存在数组变量C<@_>里。举例来说，调用函数时传入两个
参数，那么第一个参数存在C<$_[0]>里，第二个参数存在C<$_[1]>里。数组变量
C<@_>是局部变量，但每个数组元素都是相应的实参的别名。举例来说，如果改变
数组元素C<$_[0]>，相应的实参也会改变（也有例外，如果该实参不是变量，会报
错）。如果函数的参数是数组或哈希变量的一个元素，且该函数被调用时数组或哈
希的元素并不存在，那么在函数中修改参数会创建对应的数组或哈希变量的元素。
（某些早期版本的Perl不管有没有在函数中修改参数都会创建对应的数组或哈希变
量的元素。）给整个数组变量C<@_>赋值将会解除别名，将无法修改任何参数。
...
函数中最后一个表达式的值即是函数的返回值，空函数的返回值是一个空列表。详
细来说，可以用C<return>语句退出函数，该语句可指定可选的返回值作为参数，
该参数会相应的根据函数调用时的上下文（列表，标量或空值上下文）而求值。如
果没有指定返回值，函数在列表上下文下返回空列表，在标量上下文下返回
C<undef>，在空值上下文下什么都不返回。如果返回一个或多个聚合类型（数组、
哈希）的值，该数组的元素或哈希的键值将会成为一个大列表的元素而返回。
...
Perl不能给形参命名。实际使用中，你能做的就是在函数中把参数列表赋值给一个
C<my()>列表（注：以此来命名参数）。没有被声明为私有变量的变量是全局变量。
声明私有变量的细节可参考L<"用my()声明私有变量">和L<"local()和临时值">。
需要把相关的函数放到一个单独的包里（可以是一个单独的文件），可参考
L<perlmod/"包">。
...
因为赋值的关系，该语句的另一个作用是由传入参数的引用变为传入参数的值。不
然，函数可以通过修改C<@_>来改变参数的值。
...
当然，这样不能修改常量参数。如果参数是字面量，而试图在函数中改变它的话，
将会出错。例如，以下代码是错误的：
...
在函数C<upcase_in()>中返回参数的拷贝，而不是直接修改C<@_>，是更安全的做
法。
...
注意，（没有原型定义的）函数并不关心传给它的是标量或数组。对于函数来说，
所有的参数都存放在列表C<@_>中。这种简单的传参数方式是Perl语言的闪光点。
函数C<upcase()>不需做任何更改，以下示例没有任何问题：
...
类似扁平的参数列表，函数的返回值列表也是扁平的。因此，以上示例将会把返回
值存在数组C<@a>里，且置空数组C<@b>。另参考L<引用传参>。
...
调用函数可在函数名前显式的加C<&>前缀。较新版本的Perl C<&>前缀是可选的，
正如函数有预定义的情况下圆括号也是可选的。仅当用做指名函数时C<&>I<不是>
可选的，例如作为defined()或undef()的参数时。另外，通过函数名或函数引用来
间接调用函数时C<&>前缀也不是可选的，例如C<&$subref()>或C<&{$subref}()>
这样的形式，不过也可以用C<< $subref->() >>这种写法。参考L<perlref>。
...
函数可被递归的调用。如果函数调用时使用C<&>前缀，参数列表是可选的，如果没
有参数列表，该函数会沿被调前的C<@_>数组变量。这是一高效的机制，新手也许
会避免使用。
...
efficiency mechanism that new users may wish to avoid.
    ...
    &foo;       # 当前的@_数组被传入，效果同foo(@_)!!
    foo;        # 效果同foo() 当且仅当函数foo有预定义，不然的话是一个裸字，同"foo"

C<&>前缀不仅使参数列表成为可选的，而且会禁止函数参数的原型检查。这样做有
历史的因素，亦是一种便利的方式让你可以逃过参数的原型检查（如果你知道这么
做的原因）。参考L<原型>。
...
全部由大写字母命名的函数是Perl内部保留使用的，就像全部由小写字母命名的模
块一样。运行时系统会间接的调用这样命名的函数，通常因为一个事件被触发。这
些特殊的函数有C<AUTOLOAD>，C<CLONE>，C<DESTROY>和所有在L<perltie>和
L<PeroIO::via>中提到的函数。
...
C<BEGIN>，C<CHECK>，C<INIT>和C<END>更像是有名的特殊代码块，一个包里面可
以有多个这样的代码块，它们B<不能>被直接调用。参考L<perlmod/"BEGIN, CHECK, INIT和END">。
...
B<警告>：C<my>申明语句中的属性列表还在不断的演进中。当前的语义和接口可能
会改变。参考L<attributes>和L<Attribute::Handlers>。
...
C<my>操作符申明的变量列表作用域限制在包围的代码块里，例如，条件语句
(C<if/unless/elsif/else>)，循环语句(C<for/foreach/while/until/continue>)，
函数，C<eval>或由C<do/require/use>导入的文件。如果C<my>后面跟了多个量，
必须用圆括号括起来。C<my>列表里的每个元素必须是合法的左值。只有普通的标
志符可以词法作用域，内置特殊标志符，比如C<$/>目前必须用C<local>。
...
不象C<local>操作符创建的动态作用域变量，C<my>声明的词法作用域变量对外部
是绝对隐藏的，包括任何该作用域内被调用的函数。递归调用也是如此--每此调用
都会得到新的拷贝。
...
这并不意味着C<my>声明的变量在词法作用域内不可见（？？？）。只有动态作用
域才是如此。例如，下面C<bumpx()>函数能访问词法作用域变量$x，因为C<my>和
C<sub>存在于同一个作用域，文件作用域。
